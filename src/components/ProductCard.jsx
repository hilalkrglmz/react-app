...
const updateFunc = () => {
  ...
  return (
    <div className='w-[200px] h-[200px] relative m-2 rounded-md'>
      ...
      <div className='absolute left-0 bottom-0 px-2 bg-indigo-600 text-white w-full'>
      ...
      {openEdit && (
       <div className='bg-black border border-white text-white absolute top-5 right-2 p-2 text-sm'>
        <div onClick={() => dispatch(deleteDataFunc(dt?.id))} className='cursor-pointer'>SİL</div>
        <div onClick={() => dispatch(updateFunc)} className='cursor-pointer'>GÜNCELLE</div>
       </div> 
      )}
    </div>
  )
